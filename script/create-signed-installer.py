...
def main():
    ...
    root_out_dir = os.path.join(
        ...
        args.root_out_dir[0])
    create_signed_installer(root_out_dir, args.dissenter_installer_exe[0], args.skip_signing)


def parse_args():
    parser = argparse.ArgumentParser(description='Create signed installer')
    parser.add_argument('--root_out_dir', nargs=1)
    parser.add_argument('--dissenter_installer_exe', nargs=1)
    parser.add_argument('--skip_signing', action='store_true')
    return parser.parse_args()


def create_signed_installer(root_out_dir, dissenter_installer_exe, skip_signing, env=None):
